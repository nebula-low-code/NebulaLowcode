<template>
  <div class="column-config">
    <a-page-header style="padding: 0" title="表格子列" @back="store.closeTableColumnChildrenConfig()" />
    ...
    <ColumnChildrenConfigBasic v-if="state.columnItem.choose" :columnItem="state.columnItem" />
  </div>
</template>
...
import ColumnChildrenConfigBasic from './table-column-children-config-basic.vue'
import ColumnConfigMore from './table-column-config-more.vue'
const store = useDataStore()
const curColumn = computed(() => store.curColumn)
console.log('------columnsConfigList----', store.curColumn)
let state = reactive({
